...
//!
//! `session` pins chunking invariance, the bounded input memory of a long
//! stream (measured in a forked child's peak RSS) and the release-mode encode
//! budget; `feeder_parity` proves the incremental feeder selects exactly the
//! samples the batch windowing kernel selects, frame for frame, so the two
//! paths cannot drift. One object, two angles; each module keeps its own test
