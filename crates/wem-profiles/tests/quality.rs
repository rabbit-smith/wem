//! The quality axis: the interpolation kernel and the transient record family.
//!
//! `interpolation` pins the normalization, the two-step fractional index and
//! the 13-breakpoint two-channel curves against the doubles the Python
//! reference returns. `record_family` pins the transient tables materialized on
//! the same axis — the record index the curve selects, the floor record's
//! verbatim fields and the f32 words of the interpolated bands. Both read the
//! shared vectors, so they share one binary; each module keeps its own test
...
mod interpolation {
    ...
    //! tests/unit/profiles/test_quality.py) and this module must return
    ...
    const TWO_CHANNEL_DESC29: [f64; 13] = [
        -100.0, -100.0, -100.0, -100.0, -100.0, -100.0, -100.0, -105.0, -105.0, -105.0, -105.0,
        -110.0, -120.0,
    ];
    const TWO_CHANNEL_DESC30: [f64; 13] = [
        -130.0, -130.0, -130.0, -130.0, -130.0, -135.0, -140.0, -140.0, -140.0, -140.0, -140.0,
        -140.0, -150.0,
    ];
...
mod record_family {
    ...
    //! tests/unit/profiles/test_quality.py) and this module must
