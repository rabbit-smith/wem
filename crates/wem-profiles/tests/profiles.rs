//! The compiled profile carrier and the selection that resolves against it.
//!
//! One object, three angles. `carrier` reads every table region out of the
//! compiled stream and pins its values as bit patterns; `selection` pins the
//! version table, the spelling round-trip and the resolution rules, including
//! the ambiguous case the selector exists to make impossible; `two_channel`
//! pins the 2ch/48000 registration against the committed two-channel
//! container. Each module keeps its own test names and assertion messages.
...
mod carrier {
    ...
    use crate::common::{six_selection, two_channel_selection};
    ...
    fn mdct_look_oracle_values() {
        ...
        assert_eq!(
            looks[&128].bitrev,
            [
                62, 0, 30, 32, 46, 16, 14, 48, 54, 8, 22, 40, 38, 24, 6, 56, 58, 4, 26, 36, 42, 20,
                10, 52, 50, 12, 18, 44, 34, 28, 2, 60,
            ]
        );
    }
    ...
    fn short_seed_oracle_values() {
        ...
        let look =
            wem_analysis::config::make_wwise_psy_look(&ss, None, Some(&frozen.coordinate_ln))
    ...
    fn long_tables_oracle_values() {
        ...
        assert_eq!(
            ...
            [
                1024, 374481848, 372291952, 375036752, 883441760, 883449968, 883454072, 4294967062,
                5, 8, 777, 44100
            ]
        );
...
mod selection {
    ...
    fn selection_matches_on_generation_and_geometry_together() {
        ...
        let other_generation =
            ProfileKey::new(6, 44_100, "2012.1".into(), "5.1".into()).expect("key");
    ...
    fn the_free_resolvers_agree_with_the_registry() {
        for selection in [six_selection(), two_channel_selection()] {
            let resolved =
                wem_profiles::resolve_wem_profile_selection(selection).expect("resolves");
            ...
            assert!(
                wem_profiles::resolve_wem_profile_selection_quality(selection, Some(f64::NAN))
                    .is_err()
            );
        }
    }
...
mod two_channel {
    ...
    //! structured selection resolved against the profile tables compiled into the
    //! library, never a profile name or a profile tree. The setup packet is pinned
    //! by comparing its bytes against the committed two-channel reference
    ...
    use crate::common::{six_selection, two_channel_reference_dir, two_channel_selection};
    ...
    fn two_channel_profile_exposes_its_quality_curves() {
        ...
        assert_the_committed_container_carries(&compiled.setup_packet().expect("setup packet"));

        let curves = compiled
            .quality_curves()
            .expect("curves read")
            .expect("2ch registers the quality-curves table");
