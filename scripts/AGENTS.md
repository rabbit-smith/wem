...
- **Cross-check before emit**: generators assert their output against what is
  ...
  validate their rendered input's SHA-256 against the corpus manifest, and with
  `--check` against the committed WAV, before writing); the reference digest is
  verified after, not assumed. Nothing writes a value for a later run to
  compare against — that comparison belongs in a test.
