...
| Native/oracle parity | Batch, one-chunk streaming and randomized chunking are byte-identical | `make fuzz-parity`, the pinned 2ch integration comparison |
...
1. Keep both paired byte-exact results, the representative and stress corpora,
   and the profile digest chain as they are.
