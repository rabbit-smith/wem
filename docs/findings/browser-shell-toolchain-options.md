...
"NodeJS and Browsers" ([jco](https://github.com/bytecodealliance/jco)); `jco`'s
own README states that it is a work in progress: it offers no guarantees of
stability, security or support, and may make breaking changes without notice.
...
| Emscripten main/side modules | **No** | — | Emscripten's model is a main module plus Emscripten-built side modules; a module built by rustc/wasm-ld is neither. The ABI it relies on is described by its own spec document (`DynamicLinking.md`) as a work in progress with no stable ABI yet, and lld's page agrees that dynamic linking support is not yet final. Note the honest gap: the Emscripten documentation never states the prohibition in words (§9) |
...
| CI cost | One `web` job: rustup + wasm-pack action + Node 22 + `make wasm-build` and `node js/test-node.mjs` (two `wasm-pack build`s, then the Node parity test), with the built packages published as a distribution artifact ([`../../.github/workflows/web.yml`](../../.github/workflows/web.yml)) | A C shim adds clang/wasi-sdk/zig to that job; a second module adds a second build and a second built artifact; the current job is one toolchain |
...
* **The component model as a drop-in binding layer.** `wasm-tools component new`
  ...
  `jco` is component → JS only and self-declared as not yet stable; `wasm-tools
